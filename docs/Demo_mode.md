...
When accessing online.interlisp.org (OIO) using Demo Mode,  Online Medley will LOAD a lisp file that was specified in the URL through which OIO was accessed.  This lisp file is referred to as a *start-script* and the intention is that when it is LOADed it will run a demo by defining and/or LOADing additional lisp code and then starting up that demo, e.g., via a P file package command.  Before the Online Medley run is started, the start-script file is downloaded using ```wget```into a known file in the Medley file system from where it is LOADed by ONLINE-INIT. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkwMDA0MjUxNywyMTUwMzcwMTAsMTcxND
ExNjk3NF19
-->